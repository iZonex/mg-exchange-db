...
impl BandJoinCursor {
    pub fn new(
        ...
        left_key_col: usize, right_key_col: usize, band: i64,
    ) -> Self {
        let mut schema = left.schema().to_vec();
        schema.extend(right.schema().to_vec());
        Self {
            left, right_rows: Vec::new(), left_key_col, right_key_col, band, schema,
            built: false, right_source: Some(right), left_buffer: Vec::new(), out_buffer: Vec::new(),
        }
    }
    ...
    fn build(&mut self) -> Result<()> {
        ...
        while let Some(b) = right.next_batch(1024)? {
            for r in 0..b.row_count() {
                self.right_rows.push((0..b.columns.len()).map(|c| b.get_value(r, c)).collect());
            }
        }
        self.built = true;
        Ok(())
    ...
    fn key_i64(v: &Value) -> i64 {
        match v { Value::I64(n) => *n, Value::Timestamp(n) => *n, Value::F64(n) => *n as i64, _ => 0 }
    }
}
...
mod tests {
    ...
    fn band_join_within_range() {
        ...
        let rs = vec![("ts".to_string(), ColumnType::I64), ("v".to_string(), ColumnType::I64)];
