//! Except cursor — returns rows from left not present in right (set difference).
...
/// Returns rows from the left cursor that do not appear in the right cursor.
///
/// Materializes the right side into a hash set, then streams left and
/// filters out any row whose key is found in the set.
pub struct ExceptCursor {
    left: Box<dyn RecordCursor>,
    right_set: HashSet<Vec<u8>>,
    /// Tracks rows already emitted to produce set semantics (no duplicates).
...
impl RecordCursor for ExceptCursor {
...
mod tests {
    ...
    fn except_basic() {
        ...
        assert_eq!(all, vec![Value::I64(1), Value::I64(3)]);
    }
}
