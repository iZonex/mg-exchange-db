...
mod where_eq {
    ...
    macro_rules! eq {
        ($n:ident, $v:expr) => {
            ...
            fn $n() {
                assert_eq!(
                    rc(&setup(), &format!("SELECT * FROM t WHERE vi = {}", $v)),
                    1
                );
            }
        };
...
mod where_between {
    use super::*;
    macro_rules! bt {
        ($n:ident, $lo:expr, $hi:expr, $expect:expr) => {
            #[test]
            fn $n() {
                assert_eq!(
                    rc(
                        &setup(),
                        &format!("SELECT * FROM t WHERE vi BETWEEN {} AND {}", $lo, $hi)
                    ),
                    $expect
                );
            }
        ...
        ($n:ident, $v:expr) => {
            ...
            fn $n() {
                assert_eq!(
                    rc(
                        &setup(),
                        &format!("SELECT * FROM t WHERE vs = 's{:02}'", $v)
                    ),
                    1
                );
            }
...
mod where_compound {
    ...
    #[test]
    fn gt_and_lt() {
        assert_eq!(rc(&setup(), "SELECT * FROM t WHERE vi > 5 AND vi < 10"), 4);
    }
    #[test]
    fn gt_and_lt2() {
        assert_eq!(rc(&setup(), "SELECT * FROM t WHERE vi > 0 AND vi < 5"), 4);
    }
    #[test]
    fn gt_and_lt3() {
        assert_eq!(rc(&setup(), "SELECT * FROM t WHERE vi > 10 AND vi < 15"), 4);
    }
    #[test]
    fn gte_and_lte() {
        assert_eq!(
            rc(&setup(), "SELECT * FROM t WHERE vi >= 5 AND vi <= 10"),
            6
        );
    }
    #[test]
    fn gte_and_lte2() {
        assert_eq!(rc(&setup(), "SELECT * FROM t WHERE vi >= 0 AND vi <= 4"), 5);
    }
    #[test]
    fn gte_and_lte3() {
        assert_eq!(
            rc(&setup(), "SELECT * FROM t WHERE vi >= 15 AND vi <= 19"),
            5
        );
    }
    #[test]
    fn or_eq() {
        assert_eq!(rc(&setup(), "SELECT * FROM t WHERE vi = 0 OR vi = 19"), 2);
    }
    #[test]
    fn or_eq2() {
        assert_eq!(rc(&setup(), "SELECT * FROM t WHERE vi = 5 OR vi = 10"), 2);
    }
    #[test]
    fn or_eq3() {
        assert_eq!(
            rc(&setup(), "SELECT * FROM t WHERE vi = 1 OR vi = 2 OR vi = 3"),
            3
        );
    }
}
...
mod agg_where {
    ...
    cw!(
        sum_bt5_10,
        "SELECT sum(vi) FROM t WHERE vi BETWEEN 5 AND 10",
        45.0
    ); // 5+6+7+8+9+10=45
    cw!(
        cnt_bt5_10,
        "SELECT count(*) FROM t WHERE vi BETWEEN 5 AND 10",
        6.0
    );
    cw!(
        min_bt5_10,
        "SELECT min(vi) FROM t WHERE vi BETWEEN 5 AND 10",
        5.0
    );
    cw!(
        max_bt5_10,
        "SELECT max(vi) FROM t WHERE vi BETWEEN 5 AND 10",
        10.0
    );
    cw!(
        avg_bt5_10,
        "SELECT avg(vi) FROM t WHERE vi BETWEEN 5 AND 10",
        7.5
    );
    ...
    cw!(
        cnt_bt0_0,
        "SELECT count(*) FROM t WHERE vi BETWEEN 0 AND 0",
        1.0
    );
    cw!(
        cnt_bt19_19,
        "SELECT count(*) FROM t WHERE vi BETWEEN 19 AND 19",
        1.0
    );
    cw!(
        cnt_gt0_lt20,
        "SELECT count(*) FROM t WHERE vi > 0 AND vi < 20",
        19.0
    );
}
...
mod where_limit {
    use super::*;
    macro_rules! wl {
        ($n:ident, $where:expr, $lim:expr, $expect:expr) => {
            #[test]
            fn $n() {
                assert_eq!(
                    rc(
                        &setup(),
                        &format!("SELECT * FROM t WHERE {} LIMIT {}", $where, $lim)
                    ),
                    $expect
                );
            }
    ...
    wl!(gl14, "vi <= 19", 10, 10);
    wl!(gl15, "vi <= 19", 20, 20);
    wl!(gl16, "vi BETWEEN 5 AND 10", 3, 3);
    wl!(gl17, "vi BETWEEN 5 AND 10", 6, 6);
...
mod select_all {
    ...
    #[test]
    fn sum_all() {
        assert!((ff(&setup(), "SELECT sum(vi) FROM t") - 190.0).abs() < 0.01);
    }
    #[test]
    fn avg_all() {
        assert!((ff(&setup(), "SELECT avg(vi) FROM t") - 9.5).abs() < 0.01);
    }
    #[test]
    fn min_all() {
    ...
    #[test]
    fn max_all() {
        assert!((ff(&setup(), "SELECT max(vi) FROM t") - 19.0).abs() < 0.01);
    }
}
...
mod agg_where2 {
    ...
    aw!(
        sum_bt0_4,
        "SELECT sum(vi) FROM t WHERE vi BETWEEN 0 AND 4",
        10.0
    );
    aw!(
        sum_bt0_9,
        "SELECT sum(vi) FROM t WHERE vi BETWEEN 0 AND 9",
        45.0
    );
    aw!(
        sum_bt10_19,
        "SELECT sum(vi) FROM t WHERE vi BETWEEN 10 AND 19",
        145.0
    );
    aw!(
        avg_bt0_9,
        "SELECT avg(vi) FROM t WHERE vi BETWEEN 0 AND 9",
        4.5
    );
    aw!(
        avg_bt10_19,
        "SELECT avg(vi) FROM t WHERE vi BETWEEN 10 AND 19",
        14.5
    );
    aw!(
        min_bt5_15,
        "SELECT min(vi) FROM t WHERE vi BETWEEN 5 AND 15",
        5.0
    );
    aw!(
        max_bt5_15,
        "SELECT max(vi) FROM t WHERE vi BETWEEN 5 AND 15",
        15.0
    );
    aw!(
        cnt_bt0_4,
        "SELECT count(*) FROM t WHERE vi BETWEEN 0 AND 4",
        5.0
    );
    aw!(
        cnt_bt0_9,
        "SELECT count(*) FROM t WHERE vi BETWEEN 0 AND 9",
        10.0
    );
    aw!(
        cnt_bt10_19,
        "SELECT count(*) FROM t WHERE vi BETWEEN 10 AND 19",
        10.0
    );
}
...
mod where_limit2 {
    use super::*;
    macro_rules! wl {
        ($n:ident, $cond:expr, $lim:expr, $expect:expr) => {
            #[test]
            fn $n() {
                assert_eq!(
                    rc(
                        &setup(),
                        &format!("SELECT * FROM t WHERE {} LIMIT {}", $cond, $lim)
                    ),
                    $expect
                );
            }
    ...
    wl!(g01, "vi >= 5 AND vi <= 10", 1, 1);
    wl!(g02, "vi >= 5 AND vi <= 10", 3, 3);
    wl!(g03, "vi >= 5 AND vi <= 10", 6, 6);
    wl!(g04, "vi >= 5 AND vi <= 10", 10, 6);
    wl!(g05, "vi = 0", 1, 1);
    wl!(g06, "vi = 0", 5, 1);
    wl!(g07, "vi > 15", 1, 1);
    wl!(g08, "vi > 15", 4, 4);
    ...
    wl!(g11, "vi < 5", 3, 3);
    wl!(g12, "vi < 5", 5, 5);
    wl!(g13, "vi >= 0 AND vi < 10", 5, 5);
    wl!(g14, "vi >= 10 AND vi < 20", 5, 5);
    wl!(g15, "vi > 0 AND vi < 19", 10, 10);
    wl!(g16, "vi > 0 AND vi < 19", 18, 18);
    wl!(g17, "vi BETWEEN 3 AND 7", 2, 2);
    wl!(g18, "vi BETWEEN 3 AND 7", 5, 5);
    wl!(g19, "vi BETWEEN 3 AND 7", 10, 5);
    wl!(g20, "vi BETWEEN 0 AND 19", 0, 0);
}
...
mod where_str {
    use super::*;
    macro_rules! ws {
        ($n:ident, $cond:expr, $expect:expr) => {
            #[test]
            fn $n() {
                assert_eq!(
                    rc(&setup(), &format!("SELECT * FROM t WHERE {}", $cond)),
                    $expect
                );
            }
        };
...
mod where_or {
    ...
    wo!(o01, "vi = 0 OR vi = 1", 2);
    wo!(o02, "vi = 0 OR vi = 19", 2);
    wo!(o03, "vi = 5 OR vi = 10", 2);
    wo!(o04, "vi = 5 OR vi = 10 OR vi = 15", 3);
