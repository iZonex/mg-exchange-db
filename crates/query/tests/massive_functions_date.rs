...
mod now_extra {
    ...
    #[test]
    fn current_date_alias() {
        match eval("current_date", &[]) {
            Value::Timestamp(ns) => assert!(ns > 0),
            _ => panic!(),
        }
    }
    ...
    #[test]
    fn yesterday_approx_1_day_before() {
        let y = match eval("yesterday", &[]) {
            Value::Timestamp(ns) => ns,
            _ => panic!(),
        };
    ...
    #[test]
    fn tomorrow_approx_1_day_after() {
        let t = match eval("tomorrow", &[]) {
            Value::Timestamp(ns) => ns,
            _ => panic!(),
        };
...
mod to_timestamp_extra {
    ...
    #[test]
    fn date_parse_alias() {
        assert_eq!(eval("date_parse", &[s("1000")]), ts(1000));
    }
    #[test]
    fn timestamp_parse_alias() {
        assert_eq!(eval("timestamp_parse", &[s("1000")]), ts(1000));
    }
}
...
mod extract_year_extra {
    ...
    #[test]
    fn y2024_mar() {
        assert_eq!(eval("extract_year", &[ts(TS_2024_03_15_123045)]), i(2024));
    }
    #[test]
    fn y2024_jun() {
        assert_eq!(eval("extract_year", &[ts(TS_2024_06_15)]), i(2024));
    }
}
...
mod extract_month_extra {
    ...
    #[test]
    fn from_int() {
        assert_eq!(eval("extract_month", &[i(TS_2024_01_01)]), i(1));
    }
}
...
mod extract_day_extra {
    ...
    #[test]
    fn from_int() {
        assert_eq!(eval("extract_day", &[i(TS_2024_01_01)]), i(1));
    }
}
...
mod extract_hour_extra {
    ...
    #[test]
    fn from_int() {
        assert_eq!(eval("extract_hour", &[i(TS_2024_01_01)]), i(0));
    }
}
...
mod extract_misc_extra {
    ...
    fn day_of_week_alias() {
        let r = eval("day_of_week", &[ts(TS_2024_06_15)]);
        match r {
            Value::I64(_) => {}
            _ => panic!(),
        }
...
mod date_trunc_extra {
    ...
    #[test]
    fn trunc_year() {
        let r = eval("date_trunc", &[s("year"), ts(TS_2024_03_15_123045)]);
    ...
    #[test]
    fn trunc_day() {
        let r = eval("date_trunc", &[s("day"), ts(TS_2024_03_15_123045)]);
        match r {
            Value::Timestamp(v) => assert!(v < TS_2024_03_15_123045),
            _ => panic!(),
        }
    }
    #[test]
    ...
    #[test]
    fn trunc_null() {
        assert_eq!(eval("date_trunc", &[s("year"), null()]), null());
    }
    #[test]
    fn trunc_epoch() {
        assert_eq!(eval("date_trunc", &[s("year"), ts(EPOCH)]), ts(EPOCH));
    }
    #[test]
    fn timestamp_floor_alias() {
        let r = eval("timestamp_floor", &[s("year"), ts(TS_2024_03_15_123045)]);
        assert_eq!(r, ts(TS_2024_01_01));
    }
    #[test]
    fn trunc_minute() {
    ...
    fn trunc_second() {
        ...
        assert_eq!(r, ts(TS_2024_03_15_123045));
    }
    #[test]
    fn trunc_year_2000() {
        let r = eval("date_trunc", &[s("year"), ts(TS_2000_01_01)]);
        assert_eq!(r, ts(TS_2000_01_01));
    }
}
    ...
    fn diff_minutes() {
        let r = eval(
            ...
            &[
                s("minute"),
                ts(TS_2024_01_01),
                ts(TS_2024_01_01 + 120 * NANOS_PER_SEC),
            ],
        );
        match r {
    ...
    fn diff_hours() {
        let r = eval(
            ...
            &[
                s("hour"),
                ts(TS_2024_01_01),
                ts(TS_2024_01_01 + 3 * NANOS_PER_HOUR),
            ],
        );
        match r {
    ...
    fn diff_days() {
        let r = eval(
            ...
            &[
                s("day"),
                ts(TS_2024_01_01),
                ts(TS_2024_01_01 + 5 * NANOS_PER_DAY),
            ],
        );
        match r {
...
mod timestamp_add_extra {
    ...
    #[test]
    fn add_seconds() {
        let r = eval("timestamp_add", &[s("second"), i(60), ts(TS_2024_01_01)]);
        assert_eq!(r, ts(TS_2024_01_01 + 60 * NANOS_PER_SEC));
    }
    #[test]
    fn add_minutes() {
        let r = eval("timestamp_add", &[s("minute"), i(5), ts(TS_2024_01_01)]);
        assert_eq!(r, ts(TS_2024_01_01 + 5 * NANOS_PER_MIN));
    }
    #[test]
    fn add_hours() {
        let r = eval("timestamp_add", &[s("hour"), i(3), ts(TS_2024_01_01)]);
        assert_eq!(r, ts(TS_2024_01_01 + 3 * NANOS_PER_HOUR));
    }
    #[test]
    fn add_days() {
        let r = eval("timestamp_add", &[s("day"), i(10), ts(TS_2024_01_01)]);
        assert_eq!(r, ts(TS_2024_01_01 + 10 * NANOS_PER_DAY));
    }
    #[test]
    fn add_null() {
    ...
    fn add_negative() {
        ...
        assert_eq!(r, ts(TS_2024_01_01 - NANOS_PER_DAY));
    }
    #[test]
    fn dateadd_alias() {
        let r = eval("dateadd", &[s("day"), i(1), ts(TS_2024_01_01)]);
        assert_eq!(r, ts(TS_2024_01_01 + NANOS_PER_DAY));
    }
    #[test]
    fn date_add_alias() {
        let r = eval("date_add", &[s("day"), i(1), ts(TS_2024_01_01)]);
        assert_eq!(r, ts(TS_2024_01_01 + NANOS_PER_DAY));
    }
}
...
mod epoch_conversions_extra {
    ...
    #[test]
    fn nanos_epoch() {
        assert_eq!(eval("epoch_nanos", &[ts(EPOCH)]), i(0));
    }
    #[test]
    fn nanos_null() {
    ...
    #[test]
    fn seconds_basic() {
        assert_eq!(eval("epoch_seconds", &[ts(TS_2024_01_01)]), i(1704067200));
    }
    #[test]
    fn seconds_epoch() {
        assert_eq!(eval("epoch_seconds", &[ts(EPOCH)]), i(0));
    }
    #[test]
    fn seconds_null() {
    ...
    #[test]
    fn millis_basic() {
        assert_eq!(eval("epoch_millis", &[ts(TS_2024_01_01)]), i(1704067200000));
    }
    #[test]
    fn millis_null() {
    ...
    fn unix_timestamp_alias() {
        assert_eq!(eval("unix_timestamp", &[ts(TS_2024_01_01)]), i(1704067200));
    }
    #[test]
    fn to_unix_timestamp_alias() {
        assert_eq!(
            eval("to_unix_timestamp", &[ts(TS_2024_01_01)]),
            i(1704067200)
        );
    }
    #[test]
    ...
    fn nanos_to_secs_alias() {
        assert_eq!(eval("nanos_to_secs", &[ts(TS_2024_01_01)]), i(1704067200));
    }
}
...
mod date_part_extra {
    ...
    #[test]
    fn month() {
        assert_eq!(eval("date_part", &[s("month"), ts(TS_2024_06_15)]), i(6));
    }
    #[test]
    fn day() {
        assert_eq!(
            eval("date_part", &[s("day"), ts(TS_2024_03_15_123045)]),
            i(15)
        );
    }
    #[test]
...
mod weekend_business_extra {
    ...
    #[test]
    fn fri_not_weekend() {
        assert_eq!(eval("is_weekend", &[ts(TS_2024_03_15_123045)]), i(0));
    }
    #[test]
    fn weekend_null() {
    ...
    #[test]
    fn fri_is_business() {
        assert_eq!(eval("is_business_day", &[ts(TS_2024_03_15_123045)]), i(1));
    }
    #[test]
    fn business_null() {
...
mod is_leap_year_extra {
    ...
    #[test]
    fn y1970_not_leap() {
        assert_eq!(eval("is_leap_year_fn", &[ts(EPOCH)]), i(0));
    }
}
...
mod days_in_month_extra {
    ...
    #[test]
    fn jun() {
        assert_eq!(eval("days_in_month_fn", &[ts(TS_2024_06_15)]), i(30));
    }
    #[test]
    fn dec() {
        assert_eq!(eval("days_in_month_fn", &[ts(TS_2024_12_31_235959)]), i(31));
    }
    #[test]
    fn null_in() {
    ...
    #[test]
    fn mar() {
        assert_eq!(eval("days_in_month_fn", &[ts(TS_2024_03_15_123045)]), i(31));
    }
}
...
mod month_bounds_extra {
    ...
    #[test]
    fn first_jan() {
        let r = eval("first_of_month", &[ts(TS_2024_01_01)]);
        assert_eq!(r, ts(TS_2024_01_01));
    }
    #[test]
    fn first_mar() {
        let r = eval("first_of_month", &[ts(TS_2024_03_15_123045)]);
        match r {
            Value::Timestamp(v) => {
                let day = eval("extract_day", &[Value::Timestamp(v)]);
                assert_eq!(day, i(1));
            }
            _ => panic!(),
        }
...
mod year_quarter_week_bounds {
    ...
    fn start_year() {
        assert_eq!(
            eval("start_of_year", &[ts(TS_2024_06_15)]),
            ts(TS_2024_01_01)
        );
    }
    #[test]
    ...
    fn first_day_of_year_alias() {
        assert_eq!(
            eval("first_day_of_year", &[ts(TS_2024_06_15)]),
            ts(TS_2024_01_01)
        );
    }
    ...
    fn end_year() {
        ...
        match r {
            Value::Timestamp(v) => assert!(v > TS_2024_06_15),
            _ => panic!(),
        }
    }
    #[test]
    ...
    fn start_quarter_q1() {
        ...
        match r {
            Value::Timestamp(v) => assert_eq!(v, TS_2024_01_01),
            _ => panic!(),
        }
    }
    #[test]
    fn start_quarter_null() {
        assert_eq!(eval("start_of_quarter", &[null()]), null());
    }
    #[test]
    fn first_day_of_quarter_alias() {
        let r = eval("first_day_of_quarter", &[ts(TS_2024_02_29)]);
        assert_eq!(r, ts(TS_2024_01_01));
    }

    #[test]
    fn start_week() {
        let r = eval("start_of_week", &[ts(TS_2024_06_15)]);
        match r {
            Value::Timestamp(v) => assert!(v <= TS_2024_06_15),
            _ => panic!(),
        }
    ...
    fn first_day_of_week_alias() {
        let r = eval("first_day_of_week", &[ts(TS_2024_06_15)]);
        match r {
            Value::Timestamp(v) => assert!(v <= TS_2024_06_15),
            _ => panic!(),
        }
...
mod age_between_extra {
    ...
    #[test]
    fn age_same() {
        let r = eval("age", &[ts(TS_2024_01_01), ts(TS_2024_01_01)]);
        match r {
    ...
    #[test]
    fn age_null() {
        assert_eq!(eval("age", &[null(), ts(TS_2024_01_01)]), null());
    }
    ...
    #[test]
    fn years_1() {
        let r = eval("years_between", &[ts(TS_2024_06_15), ts(TS_2023_06_15)]);
        match r {
...
mod make_timestamp_extra {
    ...
    fn null_in() {
        assert_eq!(
            eval("make_timestamp", &[null(), i(1), i(1), i(0), i(0), i(0)]),
            null()
        );
    }
    #[test]
    fn epoch_make() {
        ...
        assert_eq!(r, ts(EPOCH));
    }
    #[test]
    fn leap_day() {
        let r = eval("make_timestamp", &[i(2024), i(2), i(29), i(0), i(0), i(0)]);
        assert_eq!(r, ts(TS_2024_02_29));
    }
    #[test]
    fn y2000() {
        let r = eval("make_timestamp", &[i(2000), i(1), i(1), i(0), i(0), i(0)]);
        assert_eq!(r, ts(TS_2000_01_01));
    }
}
...
mod timestamp_ceil_extra {
    ...
    #[test]
    fn ceil_year() {
        let r = eval("timestamp_ceil", &[s("year"), ts(TS_2024_03_15_123045)]);
        match r {
            Value::Timestamp(v) => assert!(v > TS_2024_03_15_123045),
            _ => panic!(),
        }
    ...
    #[test]
    fn ceil_month() {
        let r = eval("timestamp_ceil", &[s("month"), ts(TS_2024_03_15_123045)]);
        match r {
            Value::Timestamp(v) => assert!(v >= TS_2024_03_15_123045),
            _ => panic!(),
        }
    ...
    #[test]
    fn ceil_day() {
        let r = eval("timestamp_ceil", &[s("day"), ts(TS_2024_03_15_123045)]);
        match r {
            Value::Timestamp(v) => assert!(v >= TS_2024_03_15_123045),
            _ => panic!(),
        }
    ...
    #[test]
    fn ceil_null() {
        assert_eq!(eval("timestamp_ceil", &[s("year"), null()]), null());
    }
    #[test]
    fn ceil_already_aligned() {
        let r = eval("timestamp_ceil", &[s("year"), ts(TS_2024_01_01)]);
        assert_eq!(r, ts(TS_2024_01_01));
    }
}
...
mod interval_to_nanos_extra {
    ...
    #[test]
    fn five_sec() {
        assert_eq!(eval("interval_to_nanos", &[s("5s")]), i(5 * NANOS_PER_SEC));
    }
    #[test]
    fn ten_min() {
        assert_eq!(
            eval("interval_to_nanos", &[s("10m")]),
            i(10 * NANOS_PER_MIN)
        );
    }
}
...
mod format_extra {
    ...
    #[test]
    fn to_str_ts() {
        let r = eval("to_str_timestamp", &[ts(TS_2024_01_01)]);
        match r {
            Value::Str(v) => assert!(v.contains("2024")),
            _ => panic!("{r:?}"),
        }
    }
    #[test]
    fn to_str_ts_null() {
        assert_eq!(eval("to_str_timestamp", &[null()]), null());
    }
    #[test]
    ...
    #[test]
    fn strftime_alias() {
        let r = eval("strftime", &[ts(TS_2024_01_01)]);
        match r {
            Value::Str(v) => assert!(v.contains("2024")),
            _ => panic!("{r:?}"),
        }
    ...
    fn timestamp_to_str_alias() {
        let r = eval("timestamp_to_str", &[ts(TS_2024_01_01)]);
        match r {
            Value::Str(v) => assert!(v.contains("2024")),
            _ => panic!("{r:?}"),
        }
    ...
    fn format_timestamp_alias() {
        let r = eval("format_timestamp", &[ts(TS_2024_01_01)]);
        match r {
            Value::Str(v) => assert!(v.contains("2024")),
            _ => panic!("{r:?}"),
        }
    ...
    #[test]
    fn to_char_int() {
        let r = eval("to_char", &[i(42)]);
        match r {
            Value::Str(v) => assert_eq!(v, "42"),
            _ => panic!("{r:?}"),
        }
    }
    #[test]
    fn to_char_null() {
        assert_eq!(eval("to_char", &[null()]), null());
    }
    #[test]
    fn date_to_str_alias() {
        let r = eval("date_to_str", &[ts(TS_2024_01_01)]);
        match r {
            Value::Str(v) => assert!(v.contains("2024")),
            _ => panic!("{r:?}"),
        }
...
mod next_day_extra {
    ...
    fn from_monday() {
        ...
        match r {
            Value::Timestamp(v) => assert!(v > TS_2024_01_01),
            _ => panic!(),
        }
    }
    #[test]
    ...
    #[test]
    fn from_saturday() {
        let r = eval("next_day", &[ts(TS_2024_06_15), s("Monday")]);
        match r {
            Value::Timestamp(v) => assert!(v > TS_2024_06_15),
            _ => panic!(),
        }
    ...
    #[test]
    fn from_friday() {
        let r = eval("next_day", &[ts(TS_2024_03_15_123045), s("Monday")]);
        match r {
            Value::Timestamp(v) => assert!(v > TS_2024_03_15_123045),
            _ => panic!(),
        }
...
mod timezone_extra {
    ...
    #[test]
    fn to_tz_utc() {
        let r = eval("to_timezone", &[ts(TS_2024_01_01), s("UTC")]);
        match r {
            Value::Timestamp(v) => assert_eq!(v, TS_2024_01_01),
            _ => panic!("{r:?}"),
        }
    ...
    #[test]
    fn to_tz_null() {
        assert_eq!(eval("to_timezone", &[null(), s("UTC")]), null());
    }
    #[test]
    fn from_utc_basic() {
        let r = eval("from_utc", &[ts(TS_2024_01_01), s("UTC")]);
        match r {
            Value::Timestamp(_) => {}
            _ => panic!(),
        }
    ...
    #[test]
    fn from_utc_null() {
        assert_eq!(eval("from_utc", &[null(), s("UTC")]), null());
    }
    #[test]
    fn to_utc_basic() {
        let r = eval("to_utc", &[ts(TS_2024_01_01), s("UTC")]);
        match r {
            Value::Timestamp(_) => {}
            _ => panic!(),
        }
    ...
    #[test]
    fn to_utc_null() {
        assert_eq!(eval("to_utc", &[null(), s("UTC")]), null());
    }
}
...
mod cast_to_extra {
    ...
    #[test]
    fn cast_to_ts_int() {
        match eval("cast_to_timestamp", &[i(1000)]) {
    ...
    #[test]
    fn cast_to_int_from_str() {
        assert_eq!(eval("cast_to_int", &[s("42")]), i(42));
    }
    #[test]
    fn cast_to_int_null() {
        assert_eq!(eval("cast_to_int", &[null()]), null());
    }
    #[test]
    ...
    #[test]
    fn cast_to_float_null() {
        assert_eq!(eval("cast_to_float", &[null()]), null());
    }
    #[test]
    fn cast_to_str_from_int() {
        assert_eq!(eval("cast_to_str", &[i(42)]), s("42"));
    }
    #[test]
    fn cast_to_str_null() {
        assert_eq!(eval("cast_to_str", &[null()]), null());
    }
}
...
mod extra_timestamp_combos {
    ...
    #[test]
    fn day_2000() {
        assert_eq!(eval("extract_day", &[ts(TS_2000_01_01)]), i(1));
    }
    #[test]
    fn hour_2000() {
        assert_eq!(eval("extract_hour", &[ts(TS_2000_01_01)]), i(0));
    }
    #[test]
    fn min_2000() {
        assert_eq!(eval("extract_minute", &[ts(TS_2000_01_01)]), i(0));
    }
    #[test]
    fn sec_2000() {
        assert_eq!(eval("extract_second", &[ts(TS_2000_01_01)]), i(0));
    }
    ...
    #[test]
    fn month_2023() {
        assert_eq!(eval("extract_month", &[ts(TS_2023_06_15)]), i(6));
    }
    #[test]
    fn day_2023() {
        assert_eq!(eval("extract_day", &[ts(TS_2023_06_15)]), i(15));
    }

    // trunc variants on different timestamps
    #[test]
    fn trunc_year_2023() {
        let r = eval("date_trunc", &[s("year"), ts(TS_2023_06_15)]);
        match r {
            Value::Timestamp(v) => {
                let y = eval("extract_year", &[Value::Timestamp(v)]);
                assert_eq!(y, i(2023));
                let m = eval("extract_month", &[Value::Timestamp(v)]);
                assert_eq!(m, i(1));
            }
            _ => panic!(),
        }
    ...
    #[test]
    fn trunc_month_dec() {
        let r = eval("date_trunc", &[s("month"), ts(TS_2024_12_31_235959)]);
        match r {
            Value::Timestamp(v) => {
                let d = eval("extract_day", &[Value::Timestamp(v)]);
                assert_eq!(d, i(1));
            }
            _ => panic!(),
        }
    ...
    #[test]
    fn trunc_day_dec31() {
        let r = eval("date_trunc", &[s("day"), ts(TS_2024_12_31_235959)]);
        match r {
            Value::Timestamp(v) => {
                let h = eval("extract_hour", &[Value::Timestamp(v)]);
                assert_eq!(h, i(0));
            }
            _ => panic!(),
        }
    ...
    #[test]
    fn add_then_diff() {
        let added = eval("timestamp_add", &[s("day"), i(7), ts(TS_2024_01_01)]);
        let diff = eval("date_diff", &[s("day"), ts(TS_2024_01_01), added]);
        assert_eq!(diff, i(7));
    }
    #[test]
    fn add_hour_then_diff() {
        let added = eval("timestamp_add", &[s("hour"), i(24), ts(TS_2024_01_01)]);
        let diff = eval("date_diff", &[s("hour"), ts(TS_2024_01_01), added]);
        assert_eq!(diff, i(24));
    }
    #[test]
    fn add_minute_then_diff() {
        let added = eval("timestamp_add", &[s("minute"), i(120), ts(TS_2024_01_01)]);
        let diff = eval("date_diff", &[s("minute"), ts(TS_2024_01_01), added]);
        assert_eq!(diff, i(120));
    }
