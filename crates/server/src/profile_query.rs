...
fn main() {
    ...
    let queries = [
        ...
        (
            "SAMPLE BY 4h",
            "SELECT first(open) AS o, max(high) AS h, min(low) AS l, last(close) AS c, sum(volume) AS v FROM ohlcv SAMPLE BY 4h",
        ),
        (
            "LATEST ON",
    ...
    println!(
        "{:25} p50={:>10?}  min={:>10?}  p99={:>10?}",
        "apply_delta",
        times[ob_n as usize / 2],
        times[0],
        times[(ob_n as f64 * 0.99) as usize]
    );

    // best_bid
    let times = bench(ob_n, || {
        let _ = std::hint::black_box(store.best_bid());
    });
    println!(
        "{:25} p50={:>10?}  min={:>10?}",
        "best_bid",
    ...
    // best_ask
    let times = bench(ob_n, || {
        let _ = std::hint::black_box(store.best_ask());
    });
    println!(
        "{:25} p50={:>10?}  min={:>10?}",
        "best_ask",
    ...
    // spread
    let times = bench(ob_n, || {
        let _ = std::hint::black_box(store.spread());
    });
    println!(
        "{:25} p50={:>10?}  min={:>10?}",
        "spread",
    ...
    // mid_price
    let times = bench(ob_n, || {
        let _ = std::hint::black_box(store.mid_price());
    });
    println!(
        "{:25} p50={:>10?}  min={:>10?}",
        "mid_price",
    ...
    let times = bench(10_000, || {
        let _ = std::hint::black_box(store.current_snapshot(0));
    });
    println!(
        "{:25} p50={:>10?}  min={:>10?}",
        "snapshot(100 levels)", times[5000], times[0]
    );

    // apply batch of 10 deltas
    let batch: Vec<OrderBookDelta> = (0..10)
    ...
    let batch100: Vec<OrderBookDelta> = (0..100)
        .map(|i| OrderBookDelta {
            action: DeltaAction::Modify,
            side: if i % 2 == 0 {
                BookSide::Bid
    ...
    for i in 0..ops {
        store.apply_delta(&OrderBookDelta {
            ...
            order_count: (i % 20 + 1) as u32,
        });
    }
    let elapsed = start.elapsed();
    println!(
        "1M delta applies: {:?} ({:.1}M ops/sec)",
        elapsed,
