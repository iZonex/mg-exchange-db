...
mod rbac_users {
    ...
    fn create() {
        let (_d, s) = make_store();
        s.create_user(&make_user("a", "p")).unwrap();
    }
    #[test]
    fn get() {
        let (_d, s) = make_store();
        s.create_user(&make_user("a", "p")).unwrap();
        let u = s.get_user("a").unwrap().unwrap();
    ...
    fn list_sorted() {
        let (_d, s) = make_store();
        s.create_user(&make_user("c", "p")).unwrap();
        s.create_user(&make_user("a", "p")).unwrap();
        s.create_user(&make_user("b", "p")).unwrap();
        let u = s.list_users().unwrap();
        assert_eq!(u[0].username, "a");
        assert_eq!(u[1].username, "b");
        assert_eq!(u[2].username, "c");
    }
    #[test]
    fn delete() {
        let (_d, s) = make_store();
    ...
    fn hash_different_passwords() {
        let h1 = hash_password("p1");
        assert!(!verify_password("p2", &h1));
    }
    #[test]
    fn hash_same_password() {
    ...
    fn create_fifty() {
        let (_d, s) = make_store();
        for i in 0..50 {
            s.create_user(&make_user(&format!("u{i:03}"), "p")).unwrap();
        }
    ...
    fn delete_middle() {
        ...
        s.delete_user("u2").unwrap();
        assert_eq!(s.list_users().unwrap().len(), 4);
    }
}
...
mod rbac_roles {
    ...
    fn get_role() {
        ...
        s.create_role(&r).unwrap();
        let loaded = s.get_role("admin").unwrap().unwrap();
    ...
    fn multiple_permissions() {
        ...
        s.create_role(&r).unwrap();
        let loaded = s.get_role("rw").unwrap().unwrap();
        assert_eq!(loaded.permissions.len(), 4);
    }
    #[test]
    fn create_ten_roles() {
        let (_d, s) = make_store();
        ...
        assert_eq!(s.list_roles().unwrap().len(), 10);
    }
}
...
mod rbac_security {
    ...
    fn user_field() {
        let ctx = ctx_with(vec![]);
        assert_eq!(ctx.user, "test");
    }
    #[test]
    fn roles_field() {
        let ctx = ctx_with(vec![]);
        assert_eq!(ctx.roles, vec!["testrole"]);
    }
    #[test]
    fn ddl_perm() {
...
mod encryption_extra {
    ...
    fn large_1mb() {
        let c = cfg();
        let p: Vec<u8> = (0..1_000_000).map(|i| (i % 256) as u8).collect();
        let e = encrypt_buffer(&p, &c).unwrap();
        let d = decrypt_buffer(&e, &c).unwrap();
        assert_eq!(d, p);
    }
    ...
    fn hundred_bytes() {
        let c = cfg();
        let p: Vec<u8> = (0..100).map(|i| i as u8).collect();
        let e = encrypt_buffer(&p, &c).unwrap();
        let d = decrypt_buffer(&e, &c).unwrap();
        assert_eq!(d, p);
    }
    ...
    fn file_roundtrip() {
        ...
        fs::write(&pp, b"file content").unwrap();
        encrypt_file(&pp, &c).unwrap();
        decrypt_file(&pp, &c).unwrap();
        assert_eq!(fs::read(&pp).unwrap(), b"file content");
    }
    #[test]
    fn file_empty() {
        let dir = TempDir::new().unwrap();
        ...
        fs::write(&pp, b"").unwrap();
        encrypt_file(&pp, &c).unwrap();
        decrypt_file(&pp, &c).unwrap();
        assert!(fs::read(&pp).unwrap().is_empty());
    }
    #[test]
    fn file_large() {
        let dir = TempDir::new().unwrap();
        let c = cfg();
        let pp = dir.path().join("l.bin");
        let data: Vec<u8> = (0..10000).map(|i| (i % 256) as u8).collect();
        fs::write(&pp, &data).unwrap();
        encrypt_file(&pp, &c).unwrap();
        decrypt_file(&pp, &c).unwrap();
        assert_eq!(fs::read(&pp).unwrap(), data);
    }
    #[test]
    fn enc_ext() {
        let dir = TempDir::new().unwrap();
        let c = cfg();
        let pp = dir.path().join("x.bin");
        fs::write(&pp, b"data").unwrap();
        encrypt_file(&pp, &c).unwrap();
        let enc_path = dir.path().join("x.bin.enc");
...
mod replication_extra {
    ...
    fn full_sync_roundtrip() {
        let msg = ReplicationMessage::FullSyncRequired { table: "t".into() };
        let enc = protocol::encode(&msg).unwrap();
        let (dec, _) = protocol::decode(&enc).unwrap();
        assert_eq!(dec, msg);
    }
    ...
    #[test]
    fn status_resp_with_tables() {
        let mut tables = HashMap::new();
        tables.insert("trades".into(), 100u64);
        tables.insert("orders".into(), 50u64);
        let msg = ReplicationMessage::StatusResponse {
    ...
    fn config_replica() {
        let c = ReplicationConfig {
            ...
            primary_addr: Some("127.0.0.1:9000".into()),
...
mod tenant_extra {
    ...
    fn list_sorted() {
        let dir = TempDir::new().unwrap();
        let mgr = TenantManager::new(dir.path().to_path_buf());
        mgr.create_tenant(&make_tenant("c")).unwrap();
        mgr.create_tenant(&make_tenant("a")).unwrap();
        mgr.create_tenant(&make_tenant("b")).unwrap();
        let t = mgr.list_tenants().unwrap();
        assert_eq!(t[0].id, "a");
        assert_eq!(t[2].id, "c");
    }
    #[test]
    ...
    fn create_many() {
        ...
        assert_eq!(mgr.list_tenants().unwrap().len(), 50);
    }
    #[test]
    fn quota_fields() {
        let dir = TempDir::new().unwrap();
...
mod metering_extra {
    ...
    fn new_meter() {
        let dir = TempDir::new().unwrap();
        let _meter = UsageMeter::new(dir.path().to_path_buf());
    }
    #[test]
    fn record_query() {
        let dir = TempDir::new().unwrap();
        let meter = UsageMeter::new(dir.path().to_path_buf());
        meter.record_query("t1", 100, 500);
        assert_eq!(meter.get_usage("t1").queries, 1);
    }
    #[test]
    fn record_write() {
        let dir = TempDir::new().unwrap();
        let meter = UsageMeter::new(dir.path().to_path_buf());
        meter.record_write("t1", 100);
        assert_eq!(meter.get_usage("t1").rows_written, 100);
    }
    #[test]
    fn record_multiple_queries() {
        let dir = TempDir::new().unwrap();
        let meter = UsageMeter::new(dir.path().to_path_buf());
        for _ in 0..10 {
            meter.record_query("t1", 10, 100);
        }
        assert_eq!(meter.get_usage("t1").queries, 10);
        assert_eq!(meter.get_usage("t1").rows_read, 100);
    }
    #[test]
    ...
    fn persist_load() {
        ...
        {
            let meter = UsageMeter::new(dir.path().to_path_buf());
            meter.record_write("t1", 42);
            meter.persist().unwrap();
        }
        let mut meter2 = UsageMeter::new(dir.path().to_path_buf());
        meter2.load().unwrap();
        assert_eq!(meter2.get_usage("t1").rows_written, 42);
    }
    #[test]
    fn usage_zero() {
        let dir = TempDir::new().unwrap();
        let meter = UsageMeter::new(dir.path().to_path_buf());
        let snap = meter.get_usage("nonexistent");
    ...
    fn multi_tenant() {
        ...
        assert_eq!(meter.get_usage("t1").queries, 1);
        assert_eq!(meter.get_usage("t2").queries, 1);
    }
    #[test]
    fn get_all_usage() {
        let dir = TempDir::new().unwrap();
    ...
    fn bytes_scanned() {
        let dir = TempDir::new().unwrap();
        let meter = UsageMeter::new(dir.path().to_path_buf());
        meter.record_query("t1", 10, 5000);
        assert_eq!(meter.get_usage("t1").bytes_scanned, 5000);
    }
}
...
mod cluster_extra {
    ...
    fn test_cfg(id: &str) -> ClusterConfig {
        ClusterConfig {
            node_id: id.into(),
    ...
    #[test]
    fn register() {
        let mgr = ClusterManager::new(test_cfg("n1"));
        mgr.register().unwrap();
    ...
    #[test]
    fn five_nodes() {
        let mgr = ClusterManager::new(test_cfg("n0"));
        for i in 0..5 {
            mgr.add_node(ClusterNode::new(
                format!("n{i}"),
                format!("addr{i}"),
    ...
    #[test]
    fn node_role_primary() {
        let node = ClusterNode::new("n1".into(), "addr".into(), NodeRole::Primary);
        assert!(matches!(node.role, NodeRole::Primary));
    }
    #[test]
    fn node_role_replica() {
        let node = ClusterNode::new("n1".into(), "addr".into(), NodeRole::ReadReplica);
        assert!(matches!(node.role, NodeRole::ReadReplica));
    }
    #[test]
    fn node_role_coordinator() {
        let node = ClusterNode::new("n1".into(), "addr".into(), NodeRole::Coordinator);
        assert!(matches!(node.role, NodeRole::Coordinator));
    }
    #[test]
    fn route_query_unknown() {
        let mgr = ClusterManager::new(test_cfg("n1"));
        mgr.register().unwrap();
    ...
    fn add_node_dynamic() {
        let mgr = ClusterManager::new(test_cfg("n1"));
        mgr.register().unwrap();
        mgr.add_node_dynamic(ClusterNode::new(
            "n2".into(),
            "addr2".into(),
            NodeRole::ReadReplica,
        ))
...
mod failover_extra {
    ...
    fn new() {
        ...
        let fm = FailoverManager::new(cfg, Duration::from_secs(5));
        assert!(matches!(fm.current_role(), &ReplicationRole::Replica));
    }
    #[test]
    fn promote() {
        let cfg = ReplicationConfig {
        ...
        let mut fm = FailoverManager::new(cfg, Duration::from_secs(5));
        fm.promote_to_primary().unwrap();
        assert!(matches!(fm.current_role(), &ReplicationRole::Primary));
    }
    #[test]
    fn demote() {
        let cfg = ReplicationConfig {
        ...
        let mut fm = FailoverManager::new(cfg, Duration::from_secs(5));
        fm.demote_to_replica("127.0.0.1:9000").unwrap();
        assert!(matches!(fm.current_role(), &ReplicationRole::Replica));
    }
    #[test]
    fn health_check_interval() {
        ...
        let fm = FailoverManager::new(cfg, Duration::from_secs(10));
        assert_eq!(fm.health_check_interval(), Duration::from_secs(10));
    }
    #[test]
    fn config_access() {
        let cfg = ReplicationConfig {
        ...
        let fm = FailoverManager::new(cfg, Duration::from_secs(5));
        assert!(matches!(fm.config().role, ReplicationRole::Primary));
    }
}
