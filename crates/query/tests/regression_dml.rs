...
mod insert_types {
    ...
    #[test] fn insert_large_double() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 1e15)", ts(0)));
        let val = db.query_scalar("SELECT v FROM t");
        match val { Value::F64(v) => assert!((v - 1e15).abs() < 1.0), _ => panic!("expected F64") }
    }
    #[test] fn insert_small_double() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 0.000001)", ts(0)));
        let val = db.query_scalar("SELECT v FROM t");
        match val { Value::F64(v) => assert!((v - 0.000001).abs() < 1e-9), _ => panic!("expected F64") }
    }
    #[test] fn insert_negative_double() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, -99.99)", ts(0)));
        let val = db.query_scalar("SELECT v FROM t");
        match val { Value::F64(v) => assert!((v - (-99.99)).abs() < 0.01), _ => panic!("expected F64") }
    }
    #[test] fn insert_long_string() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v VARCHAR)");
    ...
    #[test] fn insert_string_with_spaces() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v VARCHAR)");
    ...
    #[test] fn insert_multiple_cols() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, a DOUBLE, b VARCHAR, c INT)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 1.5, 'test', 42)", ts(0)));
        let (cols, rows) = db.query("SELECT * FROM t");
        assert_eq!(cols.len(), 4);
        assert_eq!(rows.len(), 1);
    }
    #[test] fn insert_timestamp_as_nanos() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
    ...
    #[test] fn insert_max_i64() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v BIGINT)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 9223372036854775807)", ts(0)));
        let val = db.query_scalar("SELECT v FROM t");
        assert_eq!(val, Value::I64(i64::MAX));
    }
    #[test] fn insert_with_named_columns() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, a DOUBLE, b INT)");
        db.exec_ok(&format!("INSERT INTO t (timestamp, a, b) VALUES ({}, 1.0, 2)", ts(0)));
        let (_, rows) = db.query("SELECT a, b FROM t");
        assert_eq!(rows[0][0], Value::F64(1.0));
        assert_eq!(rows[0][1], Value::I64(2));
    }
    #[test] fn insert_partial_columns() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, a DOUBLE, b INT)");
        db.exec_ok(&format!("INSERT INTO t (timestamp, a) VALUES ({}, 5.0)", ts(0)));
        let (_, rows) = db.query("SELECT a FROM t");
        assert_eq!(rows[0][0], Value::F64(5.0));
    }
    #[test] fn insert_five_cols() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, a DOUBLE, b DOUBLE, c DOUBLE, d DOUBLE)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 1.0, 2.0, 3.0, 4.0)", ts(0)));
        let (cols, rows) = db.query("SELECT * FROM t");
        assert_eq!(cols.len(), 5);
        assert_eq!(rows.len(), 1);
    }
    #[test] fn insert_preserves_order() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v INT)");
        for i in 0..5 {
    ...
    #[test] fn insert_same_timestamp_different_values() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
    ...
    #[test] fn insert_mixed_null_and_values() {
        let db = TestDb::new();
    ...
    #[test] fn insert_string_with_numbers() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v VARCHAR)");
    ...
    #[test] fn insert_double_precision() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 3.141592653589793)", ts(0)));
        let val = db.query_scalar("SELECT v FROM t");
        match val { Value::F64(v) => assert!((v - std::f64::consts::PI).abs() < 1e-10), _ => panic!("expected F64") }
    }
    #[test] fn insert_negative_bigint() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v BIGINT)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, -9999999999)", ts(0)));
        let val = db.query_scalar("SELECT v FROM t");
    ...
    #[test] fn insert_one_col_int() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v INT)");
        db.exec_ok(&format!("INSERT INTO t (timestamp, v) VALUES ({}, 7)", ts(0)));
        let val = db.query_scalar("SELECT v FROM t");
        assert_eq!(val, Value::I64(7));
    }
    #[test] fn insert_returns_affected_1() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
...
mod insert_batch {
    ...
    #[test] fn batch_10_varchar() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v VARCHAR)");
        let vals: Vec<String> = (0..10).map(|i| format!("({}, 'val_{}')", ts(i), i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(10));
    }
    #[test] fn batch_10_int() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v INT)");
    ...
    #[test] fn batch_multi_col_10() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, a DOUBLE, b VARCHAR)");
        let vals: Vec<String> = (0..10).map(|i| format!("({}, {}.0, 's{}')", ts(i), i, i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(10));
    }
    #[test] fn sequential_inserts_10() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..10 {
    ...
    #[test] fn sequential_inserts_100() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..100 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i));
    ...
    #[test] fn batch_with_nulls() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        let vals: Vec<String> = (0..10).map(|i| {
            if i % 3 == 0 { format!("({}, NULL)", ts(i)) } else { format!("({}, {}.0)", ts(i), i) }
        }).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(10));
    }
    #[test] fn sum_after_batch_insert() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        let vals: Vec<String> = (1..=10).map(|i| format!("({}, {}.0)", ts(i), i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
        let val = db.query_scalar("SELECT sum(v) FROM t");
        match val { Value::F64(v) => assert!((v - 55.0).abs() < 0.01), _ => panic!("expected F64") }
    }
    #[test] fn avg_after_batch_insert() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        let vals: Vec<String> = (1..=10).map(|i| format!("({}, {}.0)", ts(i), i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
        let val = db.query_scalar("SELECT avg(v) FROM t");
        match val { Value::F64(v) => assert!((v - 5.5).abs() < 0.01), _ => panic!("expected F64") }
    }
    #[test] fn min_after_batch_insert() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        let vals: Vec<String> = (1..=10).map(|i| format!("({}, {}.0)", ts(i), i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
    ...
    #[test] fn max_after_batch_insert() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        let vals: Vec<String> = (1..=10).map(|i| format!("({}, {}.0)", ts(i), i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
    ...
    #[test] fn batch_3_cols_50() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, a INT, b DOUBLE, c VARCHAR)");
        let vals: Vec<String> = (0..50).map(|i| format!("({}, {}, {}.0, 'r{}')", ts(i), i, i * 10, i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(50));
    }
    #[test] fn batch_mixed_inserts() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        // First batch
    ...
    #[test] fn batch_then_query_order() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        let vals: Vec<String> = (0..10).map(|i| format!("({}, {}.0)", ts(i), 10 - i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
        let (_, rows) = db.query("SELECT v FROM t ORDER BY v");
        assert_eq!(rows[0][0], Value::F64(1.0));
        assert_eq!(rows[9][0], Value::F64(10.0));
    }
    #[test] fn batch_then_where() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        let vals: Vec<String> = (0..20).map(|i| format!("({}, {}.0)", ts(i), i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
    ...
    #[test] fn batch_then_group() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, g VARCHAR, v DOUBLE)");
        let vals: Vec<String> = (0..12).map(|i| {
            let g = if i % 3 == 0 { "A" } else if i % 3 == 1 { "B" } else { "C" };
            format!("({}, '{}', {}.0)", ts(i), g, i)
        }).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
    ...
    #[test] fn batch_insert_returns_count() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        let vals: Vec<String> = (0..5).map(|i| format!("({}, {}.0)", ts(i), i)).collect();
...
mod update_set {
    ...
    fn db_update() -> TestDb {
        ...
        for i in 0..10 {
            ...
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, '{}', {}.0, '{}')", ts(i), name, i * 10, cat));
        }
        db
    }

    #[test] fn set_double_all() {
        let db = db_update();
    ...
    #[test] fn set_where_eq_string() {
        let db = db_update();
        db.exec_ok("UPDATE t SET val = 999.0 WHERE cat = 'A'");
        let (_, rows) = db.query("SELECT val FROM t WHERE cat = 'A'");
        for row in &rows { assert_eq!(row[0], Value::F64(999.0)); }
    }
    #[test] fn set_where_eq_double() {
        let db = db_update();
    ...
    #[test] fn update_no_match_no_change() {
        let db = db_update();
    ...
    #[test] fn update_where_complex() {
        ...
        assert!(match rows[0][0] { Value::I64(n) => n >= 1, _ => false });
    }
    #[test] fn update_on_trades() {
    ...
    #[test] fn update_double_where_string() {
        let db = TestDb::with_trades(20);
        ...
        for row in &rows { assert_eq!(row[0], Value::F64(0.0)); }
    }
    #[test] fn update_preserves_other_cols() {
        let db = db_update();
...
mod delete_ops {
    ...
    fn db_delete() -> TestDb {
        ...
        for idx in 0..10 {
            ...
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}, {}.0, '{}')", ts(idx as i64), idx, idx * 10, s));
        }
        db
    }

    #[test] fn delete_eq_int() {
        let db = db_delete();
        db.exec_ok("DELETE FROM t WHERE i = 5");
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(9));
    }
    #[test] fn delete_eq_string() {
        let db = db_delete();
    ...
    #[test] fn delete_and() {
        ...
        match total { Value::I64(n) => assert!(n < 10), _ => panic!("expected I64") }
    }
    #[test] fn delete_or() {
        let db = db_delete();
    ...
    #[test] fn delete_from_trades_by_symbol() {
        ...
        let symbols: Vec<String> = rows.iter().map(|r| match &r[0] { Value::Str(s) => s.clone(), _ => panic!() }).collect();
        assert!(!symbols.contains(&"SOL/USD".to_string()));
    }
    #[test] fn delete_from_trades_by_side() {
        let db = TestDb::with_trades(30);
        let before = match db.query_scalar("SELECT count(*) FROM trades") { Value::I64(n) => n, _ => panic!() };
        db.exec_ok("DELETE FROM trades WHERE side = 'buy'");
        let after = match db.query_scalar("SELECT count(*) FROM trades") { Value::I64(n) => n, _ => panic!() };
        assert!(after < before);
    }
    #[test] fn delete_where_gt_double() {
        let db = TestDb::with_trades(30);
        db.exec_ok("DELETE FROM trades WHERE price > 50000");
        let (_, rows) = db.query("SELECT count(*) FROM trades WHERE price > 50000");
        assert_eq!(rows[0][0], Value::I64(0));
    }
    #[test] fn delete_idempotent() {
        let db = db_delete();
        db.exec_ok("DELETE FROM t WHERE i = 5");
        db.exec_ok("DELETE FROM t WHERE i = 5"); // second time, no match
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(9));
    }
    #[test] fn delete_complex_where() {
        let db = db_delete();
        db.exec_ok("DELETE FROM t WHERE (s = 'alpha' OR s = 'beta') AND i > 3");
        let remaining = db.query_scalar("SELECT count(*) FROM t");
        match remaining { Value::I64(n) => assert!(n < 10), _ => panic!("expected I64") }
    }
    #[test] fn delete_verify_sum_remaining() {
        let db = db_delete();
        db.exec_ok("DELETE FROM t WHERE d > 50.0");
    ...
    #[test] fn delete_all_then_insert_works() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..5 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i));
...
mod truncate {
    ...
    #[test] fn truncate_then_insert_batch() {
        ...
        db.exec_ok("TRUNCATE TABLE t");
        let vals: Vec<String> = (0..5).map(|i| format!("({}, {}.0)", ts(i + 100), i)).collect();
        db.exec_ok(&format!("INSERT INTO t VALUES {}", vals.join(", ")));
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(5));
    }
    #[test] fn truncate_preserves_schema() {
        let db = TestDb::with_trades(10);
        db.exec_ok("TRUNCATE TABLE trades");
        db.exec_ok(&format!(
            "INSERT INTO trades VALUES ({}, 'ETH/USD', 3000.0, 1.0, 'sell')", ts(0)
        ));
        let (cols, rows) = db.query("SELECT * FROM trades");
        assert_eq!(cols.len(), 5);
        assert_eq!(rows.len(), 1);
    }
    #[test] fn truncate_then_group_by() {
        let db = TestDb::with_trades(10);
        db.exec_ok("TRUNCATE TABLE trades");
        let (_, rows) = db.query("SELECT symbol, count(*) FROM trades GROUP BY symbol");
        assert_eq!(rows.len(), 0);
    }
    #[test] fn truncate_then_distinct() {
        let db = TestDb::with_trades(10);
        db.exec_ok("TRUNCATE TABLE trades");
        let (_, rows) = db.query("SELECT DISTINCT symbol FROM trades");
        assert_eq!(rows.len(), 0);
    }
    #[test] fn truncate_double_table() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..5 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i));
    ...
    #[test] fn truncate_then_multiple_inserts() {
        let db = TestDb::with_trades(10);
        db.exec_ok("TRUNCATE TABLE trades");
        for i in 0..3 {
            db.exec_ok(&format!(
                "INSERT INTO trades VALUES ({}, 'BTC/USD', {}.0, 1.0, 'buy')", ts(i), i * 1000
            ));
        }
        assert_eq!(db.query_scalar("SELECT count(*) FROM trades"), Value::I64(3));
    }
    #[test] fn truncate_one_table_other_unaffected() {
        let db = TestDb::with_trades_and_quotes();
        db.exec_ok("TRUNCATE TABLE trades");
        assert_eq!(db.query_scalar("SELECT count(*) FROM trades"), Value::I64(0));
        assert_eq!(db.query_scalar("SELECT count(*) FROM quotes"), Value::I64(20));
    }
    #[test] fn truncate_then_where() {
        let db = TestDb::with_trades(10);
        db.exec_ok("TRUNCATE TABLE trades");
        let (_, rows) = db.query("SELECT * FROM trades WHERE symbol = 'BTC/USD'");
        assert_eq!(rows.len(), 0);
    }
    #[test] fn truncate_then_order_by() {
        let db = TestDb::with_trades(10);
        db.exec_ok("TRUNCATE TABLE trades");
        let (_, rows) = db.query("SELECT * FROM trades ORDER BY price");
        assert_eq!(rows.len(), 0);
    }
    #[test] fn truncate_then_limit() {
        let db = TestDb::with_trades(10);
        db.exec_ok("TRUNCATE TABLE trades");
        let (_, rows) = db.query("SELECT * FROM trades LIMIT 100");
        assert_eq!(rows.len(), 0);
    }
    #[test] fn truncate_varchar_table() {
        let db = TestDb::new();
    ...
    #[test] fn truncate_mixed_type_table() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, a DOUBLE, b VARCHAR, c INT)");
        for i in 0..5 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0, 'x', {})", ts(i), i, i));
        }
        db.exec_ok("TRUNCATE TABLE t");
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(0));
    }
    #[test] fn truncate_large_table() {
        let db = TestDb::with_trades(100);
        db.exec_ok("TRUNCATE TABLE trades");
        assert_eq!(db.query_scalar("SELECT count(*) FROM trades"), Value::I64(0));
    }
    #[test] fn truncate_then_insert_verify_data() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..5 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i));
    ...
    #[test] fn truncate_then_insert_then_aggregate() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..10 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i));
        ...
        db.exec_ok("TRUNCATE TABLE t");
        for i in 0..3 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i + 100), (i + 1) * 10));
        }
        let val = db.query_scalar("SELECT sum(v) FROM t");
        match val { Value::F64(v) => assert!((v - 60.0).abs() < 0.01), _ => panic!("expected F64") }
    }
}
...
mod insert_errors {
    ...
    #[test] fn insert_wrong_column_count() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
...
mod dml_sequences {
    ...
    #[test] fn truncate_insert_update_select() {
        let db = TestDb::with_trades(10);
        db.exec_ok("TRUNCATE TABLE trades");
        db.exec_ok(&format!("INSERT INTO trades VALUES ({}, 'BTC/USD', 1.0, 1.0, 'buy')", ts(0)));
        db.exec_ok("UPDATE trades SET price = 99.0");
        let val = db.query_scalar("SELECT price FROM trades");
        assert_eq!(val, Value::F64(99.0));
    }
    #[test] fn insert_10_delete_5_count() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..10 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i));
    ...
    #[test] fn update_delete_insert_cycle() {
        let db = TestDb::new();
    ...
    #[test] fn insert_100_update_all_delete_half() {
        let db = TestDb::with_trades(100);
        db.exec_ok("UPDATE trades SET side = 'neutral'");
        db.exec_ok("DELETE FROM trades WHERE symbol = 'SOL/USD'");
        let count = db.query_scalar("SELECT count(*) FROM trades");
        match count { Value::I64(n) => assert!(n < 100 && n > 50), _ => panic!("expected I64") }
    }
    #[test] fn insert_delete_all_insert_again() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..5 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i));
        ...
        db.exec_ok("DELETE FROM t WHERE v >= 0.0");
        for i in 0..3 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i + 100), (i + 10)));
        }
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(3));
    }
    #[test] fn update_partial_then_delete_partial() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE, s VARCHAR)");
        for i in 0..6 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0, '{}')", ts(i), i, if i < 3 { "A" } else { "B" }));
        }
        db.exec_ok("UPDATE t SET v = 99.0 WHERE s = 'A'");
        db.exec_ok("DELETE FROM t WHERE s = 'B'");
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(3));
        let (_, rows) = db.query("SELECT v FROM t ORDER BY v");
        for row in &rows { assert_eq!(row[0], Value::F64(99.0)); }
    }
    #[test] fn insert_update_count_unchanged() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..10 {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i));
    ...
    #[test] fn truncate_insert_truncate() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..5 { db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i)); }
        db.exec_ok("TRUNCATE TABLE t");
        for i in 0..3 { db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i + 100), i)); }
        db.exec_ok("TRUNCATE TABLE t");
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(0));
    }
    #[test] fn interleaved_insert_delete() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 1.0)", ts(0)));
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 2.0)", ts(1)));
    ...
    #[test] fn update_then_verify_with_agg() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        ...
        let val = db.query_scalar("SELECT sum(v) FROM t");
        // 1 + 2 + 3 + 10 + 10 = 26
        match val { Value::F64(v) => assert!((v - 26.0).abs() < 0.01), _ => panic!("expected F64") }
    }
    #[test] fn delete_then_verify_with_agg() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 1..=5 { db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i)); }
        ...
        let val = db.query_scalar("SELECT sum(v) FROM t");
        // 1 + 2 + 3 = 6
        match val { Value::F64(v) => assert!((v - 6.0).abs() < 0.01), _ => panic!("expected F64") }
    }
    #[test] fn update_all_delete_all_is_empty() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..5 { db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i)); }
        db.exec_ok("UPDATE t SET v = 0.0");
    ...
    #[test] fn chain_insert_update_delete_verify() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE, s VARCHAR)");
        // Insert 5 rows
        for i in 0..5 { db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0, 'orig')", ts(i), i)); }
        // Update some
    ...
    #[test] fn insert_same_then_update_one() {
        let db = TestDb::new();
    ...
    #[test] fn insert_20_delete_gt_update_rest() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for i in 0..20 { db.exec_ok(&format!("INSERT INTO t VALUES ({}, {}.0)", ts(i), i)); }
