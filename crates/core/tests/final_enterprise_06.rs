    ...
    macro_rules! cd {
        ($n:ident, $name:expr) => {
            ...
            fn $n() {
                let (_d, s) = make_store();
                s.create_user(&make_user($name)).unwrap();
                s.delete_user($name).unwrap();
                assert!(s.get_user($name).unwrap().is_none());
            }
        };
    }
    cd!(cd01, "cd01");
    ...
    macro_rules! cn {
        ($n:ident, $count:expr) => {
            ...
            fn $n() {
                let (_d, s) = make_store();
                for i in 0..$count {
                    s.create_user(&make_user(&format!("y{i:03}"))).unwrap();
                }
...
mod rbac_roles_f06 {
    ...
    fn del_role() {
        ...
        .unwrap();
        s.delete_role("dr").unwrap();
        assert!(s.get_role("dr").unwrap().is_none());
    }
    #[test]
    fn del_role_nx() {
        let (_d, s) = make_store();
        assert!(s.delete_role("nope").is_err());
    }
    #[test]
    fn get_role_nx() {
        let (_d, s) = make_store();
        assert!(s.get_role("nope").unwrap().is_none());
    }
    #[test]
    fn multi_perms() {
        let (_d, s) = make_store();
...
mod rbac_ctx_f06 {
    ...
    admin_read!(ar_t1, "t1");
    admin_read!(ar_t2, "t2");
    admin_read!(ar_trades, "trades");
    admin_read!(ar_orders, "orders");
    admin_read!(ar_quotes, "quotes");
    admin_read!(ar_ticks, "ticks");
    admin_read!(ar_candles, "candles");
    admin_read!(ar_positions, "positions");
    admin_read!(ar_accounts, "accounts");
    ...
    read_spec!(rs_t1, "t1");
    read_spec!(rs_t2, "t2");
    read_spec!(rs_trades, "trades");
    read_spec!(rs_orders, "orders");
    read_spec!(rs_quotes, "quotes");

    // Read specific: wrong table denied
    macro_rules! read_deny {
        ($n:ident, $perm:expr, $query:expr) => {
            #[test]
            fn $n() {
                assert!(
                    !ctx_with(vec![Permission::Read {
                        table: Some($perm.into())
                    }])
                    .can_read_table($query)
                );
            }
        };
    }
    read_deny!(rd_01, "trades", "orders");
    read_deny!(rd_02, "orders", "trades");
    read_deny!(rd_03, "quotes", "ticks");
    read_deny!(rd_04, "ticks", "candles");
    read_deny!(rd_05, "candles", "trades");
    read_deny!(rd_06, "t1", "t2");
    read_deny!(rd_07, "t2", "t1");
    read_deny!(rd_08, "alpha", "beta");
    read_deny!(rd_09, "foo", "bar");
    read_deny!(rd_10, "x", "y");
    ...
    admin_write!(aw_trades, "trades");
    admin_write!(aw_orders, "orders");
    admin_write!(aw_quotes, "quotes");
    admin_write!(aw_ticks, "ticks");
    ...
    write_spec!(ws_trades, "trades");
    write_spec!(ws_orders, "orders");
...
mod encryption_f06 {
    ...
    rt!(e09, b"abc");
    rt!(e10, b"abcdef");
    rt!(e11, b"The quick brown fox");
    rt!(e12, b"0123456789");
    rt!(e13, b"line1\nline2\nline3");
    rt!(e14, b"tab\there");
    rt!(e15, b"ABCDEFGHIJKLMNOPQRSTUVWXYZ");
    rt!(e16, b"!@#$%^&*()");
    rt!(e17, b"spaces  and  more");
    rt!(e18, b"single");
    rt!(e19, b"double_word");
    ...
    #[test]
    fn cfg_ok() {
        assert!(EncryptionConfig::new(EncryptionAlgorithm::Aes256Gcm, vec![0; 32]).is_ok());
    }
    #[test]
    fn cfg_bad_16() {
        assert!(EncryptionConfig::new(EncryptionAlgorithm::Aes256Gcm, vec![0; 16]).is_err());
    }
    #[test]
    fn cfg_bad_0() {
        assert!(EncryptionConfig::new(EncryptionAlgorithm::Aes256Gcm, vec![]).is_err());
    }
    #[test]
    fn cfg_bad_8() {
        assert!(EncryptionConfig::new(EncryptionAlgorithm::Aes256Gcm, vec![0; 8]).is_err());
    }
}
...
mod replication_f06 {
    ...
    wal!(w02, "t1", 200, 1);
    wal!(w03, "t1", 300, 10);
    wal!(w04, "trades", 1, 100);
    wal!(w05, "orders", 2, 200);
    wal!(w06, "quotes", 3, 300);
    wal!(w07, "ticks", 4, 400);
    wal!(w08, "candles", 5, 500);
    wal!(w09, "bars", 6, 600);
    wal!(w10, "positions", 7, 0);
    wal!(w11, "accounts", 8, 1);
    wal!(w12, "logs", 9, 2);
    wal!(w13, "events", 10, 3);
    wal!(w14, "metrics", 11, 4);
    wal!(w15, "audit", 12, 5);
    ...
    ack!(a11, "r1", "trades", 100);
    ack!(a12, "r1", "orders", 200);
    ack!(a13, "r1", "quotes", 300);
    ack!(a14, "r2", "trades", 100);
    ack!(a15, "r2", "orders", 200);
    ack!(a16, "r3", "trades", 1000);
    ack!(a17, "r4", "ticks", 500);
    ack!(a18, "r5", "candles", 50);
    ack!(a19, "r6", "bars", 999);
    ack!(a20, "r7", "positions", 42);
    ack!(a21, "r8", "accounts", 7);
    ack!(a22, "r9", "logs", 0);
    ack!(a23, "r10", "events", u64::MAX);
    ack!(a24, "r1", "metrics", 12345);
    ...
    #[test]
    fn status_req_2() {
        let enc = protocol::encode(&ReplicationMessage::StatusRequest).unwrap();
        assert!(!enc.is_empty());
    }
    ...
    sr!(sr05, 0, [("t1", 0u64)]);
    sr!(sr06, 100, [("trades", 50u64)]);
    sr!(sr07, 200, [("orders", 100u64)]);
    sr!(sr08, 300, [("quotes", 150u64)]);
    sr!(sr09, 400, [("ticks", 200u64)]);
    sr!(sr10, 500, [("candles", 250u64)]);
    sr!(sr11, 1000, [("trades", 500u64), ("orders", 300u64)]);
    sr!(
        sr12,
        2000,
        [("trades", 1000u64), ("orders", 500u64), ("quotes", 500u64)]
    );
    ...
    cl!(
        cl07,
        ReplicationMessage::FullSyncRequired { table: "t".into() }
    );
    cl!(
        cl08,
        ReplicationMessage::StatusResponse {
            position: ReplicaPosition::new()
        }
    );
    ...
    fn cfg_max_lag() {
        ...
        assert_eq!(c.max_lag_bytes, 1024);
    }
    #[test]
    fn cfg_replicas() {
        let c = ReplicationConfig {
            ...
            primary_addr: None,
            sync_mode: ReplicationSyncMode::Async,
            replica_addrs: vec!["a".into(), "b".into()],
            max_lag_bytes: 0,
            ..Default::default()
        };
        assert_eq!(c.replica_addrs.len(), 2);
    }
}
...
mod cluster_f06 {
    ...
    #[test]
    fn primary() {
        let n = ClusterNode::new("p".into(), "addr".into(), NodeRole::Primary);
        assert!(matches!(n.role, NodeRole::Primary));
    }
    #[test]
    fn read_replica() {
        let n = ClusterNode::new("r".into(), "addr".into(), NodeRole::ReadReplica);
        assert!(matches!(n.role, NodeRole::ReadReplica));
    }
    #[test]
    fn node_id() {
        let n = ClusterNode::new("s".into(), "addr".into(), NodeRole::ReadReplica);
        assert_eq!(n.id, "s");
    }
...
mod tenant_f06 {
    ...
    macro_rules! lt {
        ($n:ident, $count:expr) => {
            ...
            fn $n() {
                ...
                let mgr = TenantManager::new(dir.path().to_path_buf());
                for i in 0..$count {
                    mgr.create_tenant(&make_tenant(&format!("lt{i:03}")))
                        .unwrap();
                }
    ...
    fn name() {
        let t = make_tenant("x");
        assert_eq!(t.name, "T x");
    }
    #[test]
    fn ns() {
        let t = make_tenant("x");
        assert_eq!(t.namespace, "x");
    }
    #[test]
    fn quota() {
        let t = make_tenant("x");
...
mod metering_f06 {
    ...
    fn new_empty() {
        let (_d, m) = meter();
        assert_eq!(m.get_usage("t1").queries, 0);
    }
    #[test]
    fn record_1() {
        let (_d, m) = meter();
        m.record_query("t1", 10, 100);
        assert_eq!(m.get_usage("t1").queries, 1);
    }
    #[test]
    fn record_rows() {
        let (_d, m) = meter();
        m.record_query("t1", 10, 100);
        assert_eq!(m.get_usage("t1").rows_read, 10);
    }
    #[test]
    fn record_bytes() {
        let (_d, m) = meter();
        m.record_query("t1", 10, 100);
        assert_eq!(m.get_usage("t1").bytes_scanned, 100);
    }
    ...
    fn write_0() {
        let (_d, m) = meter();
        assert_eq!(m.get_usage("t1").rows_written, 0);
    }
    #[test]
    fn write_1() {
        let (_d, m) = meter();
        m.record_write("t1", 10);
        assert_eq!(m.get_usage("t1").rows_written, 10);
    }
    #[test]
    fn write_n() {
        let (_d, m) = meter();
        for _ in 0..5u64 {
            m.record_write("t1", 100);
        }
        assert_eq!(m.get_usage("t1").rows_written, 500);
    }
    ...
    fn multi_tenant() {
        let (_d, m) = meter();
        m.record_query("a", 1, 1);
        m.record_query("b", 2, 2);
        assert_eq!(m.get_usage("a").queries, 1);
        assert_eq!(m.get_usage("b").queries, 1);
    }
    #[test]
    fn multi_rows() {
        let (_d, m) = meter();
        m.record_query("a", 10, 0);
        m.record_query("b", 20, 0);
        assert_eq!(m.get_usage("a").rows_read, 10);
        assert_eq!(m.get_usage("b").rows_read, 20);
    }
    ...
    fn all_empty() {
        let (_d, m) = meter();
        assert!(m.get_all_usage().is_empty());
    }
    #[test]
    fn all_one() {
        let (_d, m) = meter();
        m.record_query("t1", 1, 1);
        assert_eq!(m.get_all_usage().len(), 1);
    }
    #[test]
    fn all_two() {
        let (_d, m) = meter();
        m.record_query("a", 1, 1);
        m.record_query("b", 1, 1);
        assert_eq!(m.get_all_usage().len(), 2);
    }
    #[test]
    fn all_three() {
        let (_d, m) = meter();
        m.record_query("a", 1, 1);
        m.record_query("b", 1, 1);
        m.record_query("c", 1, 1);
        assert_eq!(m.get_all_usage().len(), 3);
    }
    #[test]
    fn all_five() {
        let (_d, m) = meter();
        for i in 0..5 {
            m.record_query(&format!("t{i}"), 1, 1);
        }
        assert_eq!(m.get_all_usage().len(), 5);
    }
    #[test]
    fn all_ten() {
        let (_d, m) = meter();
        ...
        assert_eq!(m.get_all_usage().len(), 10);
    }
}
