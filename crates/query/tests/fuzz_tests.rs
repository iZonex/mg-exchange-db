...
fn generate_random_sql(rng: &mut SimpleRng, count: usize) -> Vec<String> {
    ...
    let keywords = [
        ...
        "DROP", "ALTER", "ADD", "COLUMN", "UPDATE", "SET", "DELETE", "ORDER", "BY",
        "GROUP", "HAVING", "LIMIT", "OFFSET", "AND", "OR", "NOT", "NULL", "AS",
        "JOIN", "ON", "LEFT", "RIGHT", "INNER", "OUTER", "DISTINCT", "COUNT", "AVG",
        "SUM", "MIN", "MAX", "LIKE", "IN", "BETWEEN", "EXISTS", "CASE", "WHEN",
        "THEN", "ELSE", "END", "UNION", "ALL", "DESC", "ASC", "*", ",", "(", ")",
        "=", ">", "<", ">=", "<=", "<>", "!=", "'hello'", "42", "3.14",
        "timestamp", "TIMESTAMP", "DOUBLE", "VARCHAR", "INT",
    ];
...
fn fuzz_sql_parser_no_panic() {
    ...
    eprintln!("SQL fuzz: {ok_count} ok, {err_count} errors out of {} inputs", inputs.len());
}
