...
mod math_scalars {
    ...
    #[test]
    fn power_10_2() {
        assert_f64_close(&eval("power", &[f(10.0), f(2.0)]), 100.0, 0.001);
    }
    #[test]
    fn power_x_0() {
        assert_f64_close(&eval("power", &[f(5.0), f(0.0)]), 1.0, 0.001);
    }
    #[test]
    fn power_null() {
        assert_eq!(eval("power", &[null(), f(2.0)]), null());
    }

    // log / ln
    #[test]
    fn ln_1() {
        assert_f64_close(&eval("ln", &[f(1.0)]), 0.0, 0.001);
    }
    #[test]
    fn ln_e() {
...
mod string_scalars {
    ...
    #[test]
    fn length_int() {
        assert_eq!(eval("length", &[i(123)]), i(3));
    }
    #[test]
    fn length_spaces() {
    ...
    #[test]
    fn concat_empty() {
        assert_eq!(eval("concat", &[s(""), s("x")]), s("x"));
    }
    #[test]
    fn concat_null() {
        assert_eq!(eval("concat", &[null(), s("x")]), s("x"));
    }
    ...
    #[test]
    fn repeat_0() {
        assert_eq!(eval("repeat", &[s("x"), i(0)]), s(""));
    }
    #[test]
    fn repeat_1() {
...
mod aggregates {
    ...
    fn db_doubles(vals: &[f64]) -> TestDb {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for (i, v) in vals.iter().enumerate() {
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {})", ts(i as i64), v));
        }
        db
    }
    fn db_ints(vals: &[i64]) -> TestDb {
        let db = TestDb::new();
    ...
    fn db_nullable(vals: &[Option<f64>]) -> TestDb {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v DOUBLE)");
        for (i, v) in vals.iter().enumerate() {
            let vs = match v {
            ...
            db.exec_ok(&format!("INSERT INTO t VALUES ({}, {})", ts(i as i64), vs));
        }
        db
    }
    ...
    #[test]
    fn count_with_null() {
        let db = db_nullable(&[Some(1.0), None, Some(3.0)]);
        assert_eq!(db.query_scalar("SELECT count(*) FROM t"), Value::I64(3));
    }
    #[test]
    fn count_col_with_null() {
        let db = db_nullable(&[Some(1.0), None, Some(3.0)]);
        let v = db.query_scalar("SELECT count(v) FROM t");
        match v {
    ...
    fn sum_empty() {
        let db = db_doubles(&[]);
        assert_eq!(db.query_scalar("SELECT sum(v) FROM t"), Value::Null);
    }
    #[test]
    fn sum_1() {
        let db = db_doubles(&[42.0]);
        assert_eq!(db.query_scalar("SELECT sum(v) FROM t"), Value::F64(42.0));
    }
    #[test]
    fn sum_10() {
        let db = db_doubles(&[1.0; 10]);
        assert_f64_close(&db.query_scalar("SELECT sum(v) FROM t"), 10.0, 0.01);
    }
    #[test]
    fn sum_ints() {
        let db = db_ints(&[1, 2, 3, 4, 5]);
        assert_eq!(db.query_scalar("SELECT sum(v) FROM t"), Value::I64(15));
    }
    #[test]
    fn sum_with_null() {
        let db = db_nullable(&[Some(10.0), None, Some(20.0)]);
        let v = db.query_scalar("SELECT sum(v) FROM t");
        match v {
            Value::F64(f) => assert!((f - 30.0).abs() < 0.01),
            _ => panic!("expected F64"),
        }
    }
    #[test]
    fn sum_all_null() {
        let db = db_nullable(&[None, None]);
        assert_eq!(db.query_scalar("SELECT sum(v) FROM t"), Value::Null);
    }
    #[test]
    fn sum_negative() {
        let db = db_doubles(&[-1.0, -2.0, -3.0]);
        assert_f64_close(&db.query_scalar("SELECT sum(v) FROM t"), -6.0, 0.01);
    }
    ...
    fn avg_empty() {
        let db = db_doubles(&[]);
        assert_eq!(db.query_scalar("SELECT avg(v) FROM t"), Value::Null);
    }
    #[test]
    fn avg_1() {
        let db = db_doubles(&[42.0]);
        assert_eq!(db.query_scalar("SELECT avg(v) FROM t"), Value::F64(42.0));
    }
    #[test]
    fn avg_10() {
        let db = db_doubles(&[1.0; 10]);
        assert_f64_close(&db.query_scalar("SELECT avg(v) FROM t"), 1.0, 0.01);
    }
    #[test]
    fn avg_ints() {
        let db = db_ints(&[2, 4, 6]);
        assert_f64_close(&db.query_scalar("SELECT avg(v) FROM t"), 4.0, 0.01);
    }
    #[test]
    fn avg_all_same() {
        let db = db_doubles(&[5.0; 100]);
        assert_f64_close(&db.query_scalar("SELECT avg(v) FROM t"), 5.0, 0.01);
    }
    ...
    fn min_empty() {
        let db = db_doubles(&[]);
        assert_eq!(db.query_scalar("SELECT min(v) FROM t"), Value::Null);
    }
    #[test]
    fn min_1() {
        let db = db_doubles(&[42.0]);
    ...
    #[test]
    fn min_descending() {
        let db = db_doubles(&[3.0, 2.0, 1.0]);
        assert_eq!(db.query_scalar("SELECT min(v) FROM t"), Value::F64(1.0));
    }
    #[test]
    fn min_ints() {
        let db = db_ints(&[5, 3, 8, 1, 4]);
        assert_eq!(db.query_scalar("SELECT min(v) FROM t"), Value::I64(1));
    }
    #[test]
    fn min_negative() {
        let db = db_doubles(&[-5.0, -1.0, -10.0]);
        assert_eq!(db.query_scalar("SELECT min(v) FROM t"), Value::F64(-10.0));
    }
    #[test]
    fn min_with_null() {
        let db = db_nullable(&[Some(5.0), None, Some(3.0)]);
        assert_eq!(db.query_scalar("SELECT min(v) FROM t"), Value::F64(3.0));
    }
    ...
    fn max_empty() {
        let db = db_doubles(&[]);
        assert_eq!(db.query_scalar("SELECT max(v) FROM t"), Value::Null);
    }
    #[test]
    fn max_1() {
        let db = db_doubles(&[42.0]);
    ...
    fn max_ascending() {
        let db = db_doubles(&[1.0, 2.0, 3.0]);
        assert_eq!(db.query_scalar("SELECT max(v) FROM t"), Value::F64(3.0));
    }
    #[test]
    fn max_ints() {
        let db = db_ints(&[5, 3, 8, 1, 4]);
        assert_eq!(db.query_scalar("SELECT max(v) FROM t"), Value::I64(8));
    }
    #[test]
    fn max_negative() {
        let db = db_doubles(&[-5.0, -1.0, -10.0]);
        assert_eq!(db.query_scalar("SELECT max(v) FROM t"), Value::F64(-1.0));
    }
    #[test]
    fn max_with_null() {
        let db = db_nullable(&[Some(5.0), None, Some(10.0)]);
    ...
    #[test]
    fn first_basic() {
        let db = db_doubles(&[10.0, 20.0, 30.0]);
    ...
    fn first_single() {
        let db = db_doubles(&[42.0]);
        assert_eq!(db.query_scalar("SELECT first(v) FROM t"), Value::F64(42.0));
    }
    #[test]
    fn last_single() {
        let db = db_doubles(&[42.0]);
        assert_eq!(db.query_scalar("SELECT last(v) FROM t"), Value::F64(42.0));
    }
    ...
    fn all_aggs_empty() {
        let db = db_doubles(&[]);
        let (_, r) = db.query("SELECT count(*), sum(v), avg(v), min(v), max(v) FROM t");
    ...
    fn agg_with_where() {
        ...
        let (_, r) = db.query("SELECT count(*), avg(price) FROM trades WHERE symbol = 'BTC/USD'");
        assert!(match r[0][0] {
            Value::I64(n) => n > 0,
            _ => false,
        });
    ...
    fn agg_group_by() {
        ...
        let (_, r) = db.query("SELECT symbol, count(*), sum(price), avg(price), min(price), max(price) FROM trades GROUP BY symbol ORDER BY symbol");
        assert_eq!(r.len(), 3);
    }
    ...
    fn trades_sum() {
        ...
        let v = db.query_scalar("SELECT sum(price) FROM trades");
        match v {
    ...
    fn trades_avg() {
        let db = TestDb::with_trades(20);
        let v = db.query_scalar("SELECT avg(price) FROM trades");
        match v {
            Value::F64(f) => assert!(f > 0.0),
            _ => panic!("expected F64"),
        }
    ...
    fn trades_min() {
        let db = TestDb::with_trades(20);
        let v = db.query_scalar("SELECT min(price) FROM trades");
        match v {
            Value::F64(f) => assert!(f > 0.0),
            _ => panic!("expected F64"),
        }
    ...
    fn trades_max() {
        let db = TestDb::with_trades(20);
        let v = db.query_scalar("SELECT max(price) FROM trades");
        match v {
            Value::F64(f) => assert!(f > 0.0),
            _ => panic!("expected F64"),
        }
...
mod conditionals {
    ...
    fn coalesce_first_non_null() {
        assert_eq!(eval("coalesce", &[null(), i(5)]), i(5));
    }
    #[test]
    fn coalesce_first_val() {
        assert_eq!(eval("coalesce", &[i(3), i(5)]), i(3));
    }
    #[test]
    ...
    #[test]
    fn coalesce_float() {
        assert_eq!(eval("coalesce", &[null(), f(1.0)]), f(1.0));
    }

    // nullif
    #[test]
    fn nullif_equal() {
        assert_eq!(eval("nullif", &[i(5), i(5)]), null());
    }
    #[test]
    fn nullif_not_equal() {
        assert_eq!(eval("nullif", &[i(5), i(3)]), i(5));
    }
    #[test]
    fn nullif_strings() {
    ...
    #[test]
    fn nullif_strings_diff() {
        assert_eq!(eval("nullif", &[s("a"), s("b")]), s("a"));
    }
    #[test]
    fn nullif_null_first() {
        assert_eq!(eval("nullif", &[null(), i(5)]), null());
    }
    ...
    fn case_simple() {
        ...
        let v = db.query_scalar("SELECT CASE WHEN v > 3 THEN 'big' ELSE 'small' END FROM t");
    ...
    fn case_else() {
        ...
        let v = db.query_scalar("SELECT CASE WHEN v > 3 THEN 'big' ELSE 'small' END FROM t");
        assert_eq!(v, Value::Str("small".into()));
    }
    #[test]
    fn case_no_else_null() {
        let db = TestDb::new();
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v INT)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 1)", ts(0)));
        let v = db.query_scalar("SELECT CASE WHEN v > 3 THEN 'big' END FROM t");
    ...
    fn case_numeric() {
        ...
        db.exec_ok("CREATE TABLE t (timestamp TIMESTAMP, v INT)");
        db.exec_ok(&format!("INSERT INTO t VALUES ({}, 5)", ts(0)));
        let v = db.query_scalar("SELECT CASE WHEN v > 3 THEN 1 ELSE 0 END FROM t");
    ...
    fn case_three_branches() {
        ...
        let v = db.query_scalar(
            "SELECT CASE WHEN v < 3 THEN 'low' WHEN v < 7 THEN 'mid' ELSE 'high' END FROM t",
        );
        assert_eq!(v, Value::Str("mid".into()));
    }
}
