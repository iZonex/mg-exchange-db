...
impl WindowJoinCursor {
    pub fn new(
        ...
        left_ts_col: usize, right_ts_col: usize, window_nanos: i64,
    ) -> Self {
        let mut schema = left.schema().to_vec();
        schema.extend(right.schema().to_vec());
        Self {
            left, right_rows: Vec::new(), left_ts_col, right_ts_col,
            window_nanos, schema, built: false, right_source: Some(right),
        }
    }
    ...
    fn ts_val(v: &Value) -> i64 {
        match v { Value::Timestamp(n) | Value::I64(n) => *n, _ => 0 }
    }
}
