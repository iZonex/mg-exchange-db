...
mod where_order {
    ...
    #[test]
    fn where_gt_double_order_double() {
        let db = db_mixed();
    ...
    #[test]
    fn where_multiple_and_order() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT i FROM t WHERE i > 2 AND i < 8 AND s != 'gamma' ORDER BY i");
        assert!(rows.len() >= 2);
    }
    #[test]
    fn where_all_match_order() {
        let db = db_mixed();
    ...
    #[test]
    fn where_and_three_conditions_order() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT i FROM t WHERE i > 1 AND d < 80.0 AND s != 'delta' ORDER BY i");
        assert!(rows.len() >= 3);
    }
    #[test]
    fn where_or_two_string_order() {
        let db = db_mixed();
        let (_, rows) = db.query("SELECT i FROM t WHERE s = 'alpha' OR s = 'beta' ORDER BY i");
        assert_eq!(rows.len(), 4);
    }
    #[test]
    fn where_negative_int_no_results() {
        let db = db_mixed();
        let (_, rows) = db.query("SELECT i FROM t WHERE i < 0 ORDER BY i");
        assert_eq!(rows.len(), 0);
    }
    #[test]
    fn where_double_between_order() {
        let db = db_mixed();
    ...
    #[test]
    fn order_by_two_cols_desc_asc() {
        let db = db_mixed();
        let (_, rows) = db.query("SELECT s, i FROM t ORDER BY s DESC, i");
        assert_eq!(rows.len(), 10);
    }
    #[test]
    fn where_complex_parens_order() {
        let db = db_mixed();
        let (_, rows) = db.query("SELECT i FROM t WHERE (i = 1 OR i = 2) AND d < 30.0 ORDER BY i");
        assert_eq!(rows.len(), 2);
    }
    #[test]
    fn where_all_types_combined_order() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT i FROM t WHERE i > 3 AND d < 90.0 AND s = 'beta' ORDER BY i");
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0][0], Value::I64(7));
    }
    #[test]
    fn select_star_where_order() {
        let db = db_mixed();
...
mod where_limit {
    ...
    #[test]
    fn where_complex_or_and_limit() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT i FROM t WHERE (s = 'alpha' OR s = 'beta') AND i > 3 LIMIT 2");
        assert_eq!(rows.len(), 2);
    }
    #[test]
    fn limit_with_trades() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query("SELECT * FROM trades LIMIT 10");
        assert_eq!(rows.len(), 10);
    }
    #[test]
    fn where_order_limit_trades() {
        let db = TestDb::with_trades(50);
        let (_, rows) =
            db.query("SELECT price FROM trades WHERE symbol = 'BTC/USD' ORDER BY price LIMIT 5");
        assert_eq!(rows.len(), 5);
    }
    #[test]
    fn limit_with_multiple_projections() {
        let db = db_mixed();
    ...
    #[test]
    fn limit_after_multiple_wheres() {
        let db = db_mixed();
        let (_, rows) = db.query("SELECT i FROM t WHERE i > 2 AND i < 9 LIMIT 3");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn order_double_asc_limit_top_3() {
        let db = db_mixed();
        let (_, rows) = db.query("SELECT d FROM t ORDER BY d LIMIT 3");
        assert_eq!(rows[0][0], Value::F64(10.0));
...
mod group_agg {
    ...
    #[test]
    fn group_by_count_order_count() {
        let db = TestDb::with_trades(30);
        let (_, rows) = db
            .query("SELECT symbol, count(*) AS cnt FROM trades GROUP BY symbol ORDER BY cnt DESC");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn group_by_sum_order_sum() {
        let db = TestDb::with_trades(30);
        let (_, rows) =
            db.query("SELECT symbol, sum(price) FROM trades GROUP BY symbol ORDER BY sum(price)");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn group_by_avg_order() {
        let db = TestDb::with_trades(30);
        let (_, rows) =
            db.query("SELECT symbol, avg(price) FROM trades GROUP BY symbol ORDER BY avg(price)");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn group_by_min_order() {
        let db = TestDb::with_trades(30);
        let (_, rows) =
            db.query("SELECT symbol, min(price) FROM trades GROUP BY symbol ORDER BY min(price)");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn group_by_max_order() {
        let db = TestDb::with_trades(30);
        let (_, rows) =
            db.query("SELECT symbol, max(price) FROM trades GROUP BY symbol ORDER BY max(price)");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    ...
    #[test]
    fn multiple_aggs_same_query() {
        let db = db_mixed();
        let (_, rows) = db.query(
            "SELECT s, count(*), sum(d), avg(d), min(d), max(d) FROM t GROUP BY s ORDER BY s",
        );
        assert_eq!(rows.len(), 5);
    ...
    fn group_by_where_neq() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT s, count(*) FROM t WHERE s != 'alpha' GROUP BY s ORDER BY s");
        assert_eq!(rows.len(), 4);
    }
    #[test]
    fn group_by_double_agg() {
        let db = db_mixed();
    ...
    #[test]
    fn group_by_count_with_order_limit() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT s, count(*) AS c FROM t GROUP BY s ORDER BY c DESC LIMIT 3");
        assert_eq!(rows.len(), 3);
    }
}
...
mod group_having {
    ...
    fn having_with_limit() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT s, count(*) FROM t GROUP BY s HAVING count(*) = 2 LIMIT 3");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn having_count_gte() {
    ...
    #[test]
    fn having_gt_with_order_and_limit() {
        let db = db_mixed();
        let (_, rows) = db.query(
...
mod distinct {
    ...
    #[test]
    fn distinct_on_1_col_out_of_many() {
        let db = TestDb::with_trades(30);
    ...
    #[test]
    fn distinct_with_where_string_eq() {
        let db = TestDb::with_trades(30);
...
mod arithmetic {
    ...
    #[test]
    fn arith_in_limit_order() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT i, d + i AS combined FROM t ORDER BY combined DESC LIMIT 5");
        assert_eq!(rows.len(), 5);
    }
    #[test]
    fn arith_three_terms() {
        let db = db_mixed();
        let val = db.query_scalar("SELECT d + i + 0.5 FROM t WHERE i = 1");
        assert_f64_near(&val, 11.5, 0.01);
    }
    #[test]
    fn arith_sum_of_arith() {
        let db = db_mixed();
...
mod case_when {
    ...
    fn case_simple() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT CASE WHEN i > 5 THEN 'big' ELSE 'small' END FROM t ORDER BY i");
    ...
    #[test]
    fn case_numeric_result() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT CASE WHEN i > 5 THEN 1 ELSE 0 END AS flag FROM t ORDER BY i");
        assert_eq!(rows[0][0], Value::I64(0));
        assert_eq!(rows[9][0], Value::I64(1));
    }
    #[test]
    fn case_with_string_comparison() {
        let db = db_mixed();
        let (_, rows) = db
            .query("SELECT CASE WHEN s = 'alpha' THEN 'A' ELSE 'X' END AS code FROM t ORDER BY i");
    ...
    #[test]
    fn case_with_double_comparison() {
        let db = db_mixed();
        let (_, rows) = db.query(
    ...
    #[test]
    fn case_returns_double() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT CASE WHEN i > 5 THEN d * 2.0 ELSE d END FROM t WHERE i = 6");
    ...
    fn case_with_and() {
        let db = db_mixed();
        let (_, rows) = db
            .query("SELECT CASE WHEN i > 3 AND i < 8 THEN 'mid' ELSE 'edge' END FROM t ORDER BY i");
    ...
    fn case_in_agg() {
        let db = db_mixed();
        let val = db.query_scalar("SELECT count(CASE WHEN i > 5 THEN 1 END) FROM t");
        assert_eq!(val, Value::I64(5));
    }
    #[test]
    fn case_sum_conditional() {
        let db = db_mixed();
        let val = db.query_scalar("SELECT sum(CASE WHEN s = 'alpha' THEN d ELSE 0 END) FROM t");
        // alpha: 10 + 60 = 70
    ...
    fn case_with_limit() {
        let db = db_mixed();
        let (_, rows) = db
            .query("SELECT CASE WHEN i > 5 THEN 'big' ELSE 'small' END FROM t ORDER BY i LIMIT 3");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn case_on_trades() {
...
mod aliases {
    ...
    #[test]
    fn alias_group_having() {
        let db = db_mixed();
        let (cols, _) =
            db.query("SELECT s AS grp, count(*) AS cnt FROM t GROUP BY s HAVING count(*) = 2");
        assert!(cols.contains(&"grp".to_string()));
    }
    #[test]
    fn alias_four_cols() {
...
mod edge_cases {
    ...
    #[test]
    fn select_star_single_timestamp() {
        let db = TestDb::new();
...
mod larger_data {
    ...
    #[test]
    fn min_max_price_100() {
        let db = TestDb::with_trades(100);
        let (_, rows) = db.query("SELECT min(price), max(price) FROM trades");
        assert_eq!(rows.len(), 1);
    }
    #[test]
    fn distinct_symbols_100() {
        let db = TestDb::with_trades(100);
        let (_, rows) = db.query("SELECT DISTINCT symbol FROM trades ORDER BY symbol");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn where_and_order_100() {
        let db = TestDb::with_trades(100);
        let (_, rows) =
            db.query("SELECT price FROM trades WHERE side = 'buy' ORDER BY price DESC LIMIT 10");
        assert_eq!(rows.len(), 10);
    }
    #[test]
    fn group_having_100() {
    ...
    #[test]
    fn group_having_order_limit_200() {
        let db = TestDb::with_trades(200);
        let (_, rows) = db.query(
    ...
    #[test]
    fn select_50_with_all_features() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query(
    ...
    #[test]
    fn distinct_three_cols_50() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query("SELECT DISTINCT symbol FROM trades");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn where_gt_order_limit_50() {
        let db = TestDb::with_trades(50);
        let (_, rows) =
            db.query("SELECT price FROM trades WHERE price > 200.0 ORDER BY price LIMIT 5");
        assert_eq!(rows.len(), 5);
    }
    #[test]
    fn nested_conditions_50() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query(
    ...
    #[test]
    fn select_two_columns_order_50() {
        let db = TestDb::with_trades(50);
    ...
    #[test]
    fn group_by_alias_order_50() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db
            .query("SELECT symbol AS s, count(*) AS c FROM trades GROUP BY symbol ORDER BY c DESC");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn having_trades_avg_50() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query(
...
mod multi_feature {
    ...
    #[test]
    fn arith_where_order_limit() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT d * 2.0 AS doubled FROM t WHERE i > 5 ORDER BY doubled LIMIT 3");
        assert_eq!(rows.len(), 3);
    }
    #[test]
    fn case_group_order() {
    ...
    #[test]
    fn multiple_aggs_group_order_limit() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query(
    ...
    fn where_arith_group() {
        let db = db_mixed();
        let (_, rows) = db.query("SELECT s, sum(d * 2.0) FROM t WHERE i > 2 GROUP BY s ORDER BY s");
        assert!(rows.len() >= 3);
    }
    #[test]
    fn where_multiple_or_order_limit() {
        let db = db_mixed();
        let (_, rows) = db.query(
    ...
    fn where_not_eq_group_order() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT s, avg(d) FROM t WHERE s != 'alpha' GROUP BY s ORDER BY s");
        assert_eq!(rows.len(), 4);
    }
    #[test]
    fn case_distinct() {
    ...
    #[test]
    fn where_order_limit_1_min() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query("SELECT price FROM trades ORDER BY price LIMIT 1");
        assert_eq!(rows.len(), 1);
    }
    #[test]
    fn where_order_limit_1_max() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query("SELECT price FROM trades ORDER BY price DESC LIMIT 1");
        assert_eq!(rows.len(), 1);
    }
    #[test]
    fn group_order_by_agg_limit() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query(
    ...
    fn where_gt_lt_group_count() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT s, count(*) FROM t WHERE i >= 3 AND i <= 8 GROUP BY s ORDER BY s");
        assert!(rows.len() >= 3);
    }
    #[test]
    fn case_where_group_having_order_limit() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query(
    ...
    #[test]
    fn multiple_aliases_group_order() {
        let db = db_mixed();
        let (cols, rows) = db.query(
    ...
    #[test]
    fn where_string_order_desc_limit() {
        let db = db_mixed();
    ...
    #[test]
    fn group_multiple_aggs_having_order() {
        let db = db_mixed();
        let (_, rows) = db.query(
    ...
    #[test]
    fn distinct_where_order_limit() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db
            .query("SELECT DISTINCT symbol FROM trades WHERE side = 'buy' ORDER BY symbol LIMIT 2");
        assert_eq!(rows.len(), 2);
    }
    #[test]
    ...
    #[test]
    fn where_neq_group_count_order() {
        let db = db_mixed();
        let (_, rows) =
            db.query("SELECT s, count(*) AS c FROM t WHERE i != 1 GROUP BY s ORDER BY c DESC");
        assert!(rows.len() >= 4);
    }
    #[test]
    fn full_query_with_arith() {
        let db = TestDb::with_trades(50);
        let (_, rows) = db.query(
    ...
    #[test]
    fn where_group_multiple_having_order_limit() {
        let db = TestDb::with_trades(100);
        let (_, rows) = db.query(
    ...
    #[test]
    fn select_all_features_combined() {
        let db = TestDb::with_trades(100);
        let (_, rows) = db.query(
